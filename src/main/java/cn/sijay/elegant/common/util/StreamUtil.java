...
import cn.sijay.elegant.common.constants.SymbolConstants;
...
public class StreamUtil {
    ...
    public static <E> String join(Collection<E> collection, Function<E, String> function) {
        return join(collection, function, SymbolConstants.COMMA);
    }
     ...
     * @return 转化后的map
     */
    public static <V, K> Map<K, V> toIdentityMap(List<V> list, Function<V, K> key) {
        if (list.isEmpty()) {
            return new HashMap<>();
        }
        return list.stream().filter(Objects::nonNull).collect(Collectors.toMap(key, Function.identity(), (l, r) -> l));
    }

    /**
     * 将Collection转化为map(value类型与list的泛型不同)<br>
