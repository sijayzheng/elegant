...
import cn.sijay.elegant.system.service.SystemMessageService;
...
public class SystemMessageController extends BaseController {
     ...
     */
    @PostMapping("/add")
    public Result<String> add(@RequestBody SystemMessage entity) {
        return of(systemMessageService.add(entity), "保存");
    }
     ...
     */
    @PostMapping("/update")
    public Result<String> update(@RequestBody SystemMessage entity) {
        return of(systemMessageService.update(entity), "更新");
    }
    ...
    public Result<String> importData(@RequestPart("file") MultipartFile file) throws Exception {
        return of(systemMessageService.importData(ExcelUtil.importExcel(file.getInputStream(), SystemMessage.class)), "导入");
    }
