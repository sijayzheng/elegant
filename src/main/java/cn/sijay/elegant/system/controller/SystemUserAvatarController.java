...
import cn.sijay.elegant.system.service.SystemUserAvatarService;
...
public class SystemUserAvatarController extends BaseController {
     ...
     */
    @PostMapping("/add")
    public Result<String> add(@RequestBody SystemUserAvatar entity) {
        return of(systemUserAvatarService.add(entity), "保存");
    }
     ...
     */
    @PostMapping("/update")
    public Result<String> update(@RequestBody SystemUserAvatar entity) {
        return of(systemUserAvatarService.update(entity), "更新");
    }
    ...
    public Result<String> importData(@RequestPart("file") MultipartFile file) throws Exception {
        return of(systemUserAvatarService.importData(ExcelUtil.importExcel(file.getInputStream(), SystemUserAvatar.class)), "导入");
    }
