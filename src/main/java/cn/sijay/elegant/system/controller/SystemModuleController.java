...
import cn.sijay.elegant.system.service.SystemModuleService;
...
public class SystemModuleController extends BaseController {
     ...
     */
    @PostMapping("/add")
    public Result<String> add(@RequestBody SystemModule entity) {
        return of(systemModuleService.add(entity), "保存");
    }
     ...
     */
    @PostMapping("/update")
    public Result<String> update(@RequestBody SystemModule entity) {
        return of(systemModuleService.update(entity), "更新");
    }
    ...
    public Result<String> importData(@RequestPart("file") MultipartFile file) throws Exception {
        return of(systemModuleService.importData(ExcelUtil.importExcel(file.getInputStream(), SystemModule.class)), "导入");
    }
