...
import cn.sijay.elegant.system.service.SystemPostService;
...
public class SystemPostController extends BaseController {
     ...
     */
    @PostMapping("/add")
    public Result<String> add(@RequestBody SystemPost entity) {
        return of(systemPostService.add(entity), "保存");
    }
     ...
     */
    @PostMapping("/update")
    public Result<String> update(@RequestBody SystemPost entity) {
        return of(systemPostService.update(entity), "更新");
    }
    ...
    public Result<String> importData(@RequestPart("file") MultipartFile file) throws Exception {
        return of(systemPostService.importData(ExcelUtil.importExcel(file.getInputStream(), SystemPost.class)), "导入");
    }
