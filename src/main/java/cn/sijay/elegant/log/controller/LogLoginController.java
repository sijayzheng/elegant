...
import cn.sijay.elegant.log.service.LogLoginService;
...
public class LogLoginController extends BaseController {
     ...
     */
    @PostMapping("/add")
    public Result<String> add(@RequestBody LogLogin entity) {
        return of(logLoginService.add(entity), "保存");
    }
     ...
     */
    @PostMapping("/update")
    public Result<String> update(@RequestBody LogLogin entity) {
        return of(logLoginService.update(entity), "更新");
    }
    ...
    public Result<String> importData(@RequestPart("file") MultipartFile file) throws Exception {
        return of(logLoginService.importData(ExcelUtil.importExcel(file.getInputStream(), LogLogin.class)), "导入");
    }
