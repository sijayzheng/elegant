package cn.sijay.elegant.file.controller;
...
import cn.sijay.elegant.file.service.FileStorageService;
...
public class FileStorageController extends BaseController {
     ...
     */
    @PostMapping("/add")
    public Result<String> add(@RequestBody FileStorage entity) {
        return of(fileStorageService.add(entity), "保存");
    }
     ...
     */
    @PostMapping("/update")
    public Result<String> update(@RequestBody FileStorage entity) {
        return of(fileStorageService.update(entity), "更新");
    }
    ...
    public Result<String> importData(@RequestPart("file") MultipartFile file) throws Exception {
        return of(fileStorageService.importData(ExcelUtil.importExcel(file.getInputStream(), FileStorage.class)), "导入");
    }
